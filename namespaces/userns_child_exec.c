...
modify_individual_capability(char *optval)
{
    ...
    for (char *p = optval; p != op; p++)
    {
        ...
        case 'i':
        case 'e':
            flag = (*p == 'p') ? CAP_PERMITTED : (*p == 'e') ? CAP_EFFECTIVE
                                                             : CAP_INHERITABLE;
        ...
        case 'a':
            if (prctl(PR_CAP_AMBIENT,
                      (*op == '+') ? PR_CAP_AMBIENT_RAISE : PR_CAP_AMBIENT_LOWER,
                      cap, 0, 0) == -1)
...
set_securebits(char *optval)
{
    ...
    for (char *p = optval; *p != '\0'; p++)
    {
        ...
        case 'a':
            ...
            break;
        case 'A':
            secbits |= SECBIT_NO_CAP_AMBIENT_RAISE_LOCKED;
            break;
        case 'k':
            secbits |= SECBIT_KEEP_CAPS;
            break;
        case 'K':
        ...
        case 'r':
            ...
            break;
        case 'R':
            secbits |= SECBIT_NOROOT_LOCKED;
            break;
        case 's':
            secbits |= SECBIT_NO_SETUID_FIXUP;
            break;
        case 'S':
            secbits |= SECBIT_NO_SETUID_FIXUP_LOCKED;
            break;
        case '0':
            secbits = 0;
            break;
